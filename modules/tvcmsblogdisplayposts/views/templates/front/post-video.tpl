...
{$tmp = 1}
	{foreach from=$videos item=videourl}
        {if $tmp == 1}
        {assign var='url_parts' value=explode('?', $videourl)}
        {assign var='query_string' value=$url_parts[1]}
        {assign var='params' value=explode('&', $query_string)}
            {foreach from=$params item='param'}
                {assign var='pair' value=explode('=', $param)}
                {assign var='youtube_id' value=$pair[1]}
            {/foreach}
