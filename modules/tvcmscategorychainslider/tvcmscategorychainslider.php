    ...
    public function selectAllLangIdById($id_tvcmscategory_chain_slider)
    {
        $select_data = 'SELECT 
                        ...
                        WHERE 
                            ...
                            AND id_tvcmscategory_chain_slider = ' . (int) $id_tvcmscategory_chain_slider;
        $ans = Db::getInstance()->executeS($select_data);
        $return = [];
        foreach ($ans as $a) {
            $return[] = $a['id_lang'];
        ...
        if ($data['id']) {
            ...
            $insert_data[] = 'UPDATE `' . _DB_PREFIX_ . 'tvcmscategory_chain_slider`
                        ...
                        WHERE
                            `id_shop_group` = ' . (int) $this->id_shop_group . '
                            AND `id_shop` = ' . (int) $this->id_shop . '
                            AND `id_tvcmscategory_chain_slider` = ' . (int) $id . ';';
                    ...
                    $insert_data[] = 'UPDATE `' . _DB_PREFIX_ . 'tvcmscategory_chain_slider_lang`
                            ...
                            WHERE
                                ...
                                AND 
                                    `id_shop` = ' . (int) $this->id_shop . '
                                AND
                                    `id_tvcmscategory_chain_slider` = ' . (int) $id . '
                                AND
                                    `id_lang` = ' . (int) $lang['id_lang'] . ';';
                } else {
            ...
            $insert_data[] = 'INSERT INTO `' . _DB_PREFIX_ . 'tvcmscategory_chain_slider`
                        SET 
                            id_tvcmscategory_chain_slider = ' . (int) $new_id . ',
                            id_category = \'' . (int) $data['id_category'] . '\',
                            position = ' . (int) $new_id . ',
                            `id_shop_group` = ' . (int) $this->id_shop_group . ',
                            `id_shop` = ' . (int) $this->id_shop . ',
    ...
    public function showData($id = null)
    {
        ...
        $select_data .= 'SELECT * FROM `' . _DB_PREFIX_ . 'tvcmscategory_chain_slider` 
                ...
                `id_shop_group` = ' . (int) $this->id_shop_group
                 . ' AND `id_shop` = ' . (int) $this->id_shop;
        if ($id) {
            $select_data .= ' AND `id_tvcmscategory_chain_slider` = ' . (int) $id;
        } else {
